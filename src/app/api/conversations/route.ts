...
async function createConversation(characterId: string) {
  const id = randomUUID();
  const character = await query("SELECT name,greeting FROM characters WHERE id=$1", [characterId]);
  ...
  const greeting = String(character.rows[0].greeting || "").trim();
  if (greeting) {
...
export async function POST(request: Request) {
  ...
  const conversation = await createConversation(body.characterId);
